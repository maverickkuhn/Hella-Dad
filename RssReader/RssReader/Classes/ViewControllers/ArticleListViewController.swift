...
class ArticleListViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate, MenuViewDelegate {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if (indexPath.row + 1) % 4 != 0 {
            ...
            if var articleImageURL = article.headerImageURL {
                ...
                if articleImageURL != "" {
                    ...
                    thumbnailCell.thumbnailImageView.sd_setImageWithURL(NSURL(string: articleImageURL), completed: { (image, error, SDImageCacheType, url) -> Void in
                        if image != nil {
                            ...
                            println("Failed to load \(articleImageURL): \(error.localizedDescription)")
